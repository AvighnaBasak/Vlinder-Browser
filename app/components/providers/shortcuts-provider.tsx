...
import type { ShortcutAction } from '@/app/types/shortcuts'
...
export const ShortcutsProvider = ({ children }: ShortcutsProviderProps) => {
  ...
  const fetchShortcuts = useCallback(async () => {
    setIsLoading(true)
    try {
      const fetchedShortcuts = await conveyor.shortcuts.getShortcuts()
  ...
  useEffect(() => {
    ...
    const unsubscribe = conveyor.shortcuts.onShortcutsUpdated(() => {
      fetchShortcuts()
    })

    return () => unsubscribe()
  }, [fetchShortcuts, conveyor.shortcuts])

  const setShortcut = useCallback(
    async (actionId: string, shortcut: string): Promise<boolean> => {
      try {
        const success = await conveyor.shortcuts.setShortcut(actionId, shortcut)
        if (success) {
    ...
    [conveyor.shortcuts, fetchShortcuts]
  )

  const resetShortcut = useCallback(
    async (actionId: string): Promise<boolean> => {
      try {
        const success = await conveyor.shortcuts.resetShortcut(actionId)
        if (success) {
          fetchShortcuts() // Refresh shortcut data
        }
        return success
  ...
  const resetAllShortcuts = useCallback(async (): Promise<void> => {
    setIsLoading(true)
    try {
      const resetPromises = shortcuts.map((shortcut) => conveyor.shortcuts.resetShortcut(shortcut.id))
  ...
  }, [shortcuts, conveyor.shortcuts, fetchShortcuts])
